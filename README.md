...
- **屏幕样式**：可选择圆屏或方屏；方屏计时页使用顶部留出时钟开口的圆角矩形进度框，沿用阶段配色、大号倒计时和居中操作按钮。列表分别使用带缩放变形的 `TransformingLazyColumn` 和普通 `LazyColumn`；列表底部的返回按钮在圆屏上使用边缘按钮，在方屏上使用居中的小圆形按钮。方屏使用侧边直线滚动指示器和底部横向分页圆点，沿用圆屏指示器的配色、圆角和过渡效果；分页圆点叠放在首页底部，页面内容保持全屏高度。
...
通用设置提供屏幕样式、权限检查、系统应用设置、返回手势配置和关于页面。在 **通用设置 → 屏幕样式** 选择圆屏或方屏后，计时页及计时设置、通用设置、屏幕样式、权限和关于页面的列表立即切换，选择会保存到本地。方屏模式下还会显示 **计时进度条圆角**，使用数字选择器统一调整首页进度框四角半径（1–100 dp，默认 24 dp）；保存后立即生效，取消不保存，切换屏幕样式后仍保留数值。实际半径不超过进度框短边的一半，并保留顶部时钟开口。方屏计时布局适配正方形与长方形屏幕，暂停后仍在原位显示继续和停止按钮；圆屏计时页和时长 / 轮数选择器保持原有布局。在 Android 16 / Wear OS 6（API 36）及以上系统中，右滑返回由「系统返回手势」统一控制；较旧系统可分别设置。
...
Release 启用 R8 代码优化、混淆和资源缩减。应用不额外整包保留代码，Android 组件入口及反射需求由 AGP 和依赖自带规则处理；自定义规则目录为 [`app/src/main/keepRules/`](app/src/main/keepRules/rules.keep)，由 AGP 自动读取。

Release 仅打包英文默认资源和中文语言资源，并压缩原生库以减小 APK；安装时系统会解压原生库，因此 APK 大小不等于安装占用。构建还排除依赖版本标记和未使用的 Kotlin built-ins 元数据，保留许可证、服务注册与启动性能配置。新增语言时需同步 `locales_config.xml` 和 Release 的 `localeFilters`；若以后引入 `kotlin-reflect`，需取消对 `.kotlin_builtins` 的排除。

按设备 ABI 选择对应 APK 可进一步减小下载体积；`universal` 同时包含两种 ARM 架构。未提供签名配置的 Release APK 未签名，安装或分发前需要签名。
