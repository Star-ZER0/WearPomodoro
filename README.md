...
通用设置提供屏幕样式、权限检查、系统应用设置、返回手势配置和关于页面。在 **通用设置 → 屏幕样式** 选择圆屏或方屏后，计时页及计时设置、通用设置、屏幕样式、权限和关于页面的列表立即切换，选择会保存到本地。方屏计时布局适配正方形与长方形屏幕，暂停后仍在原位显示继续和停止按钮；圆屏计时页和时长 / 轮数选择器保持原有布局。在 Android 16 / Wear OS 6（API 36）及以上系统中，右滑返回由「系统返回手势」统一控制；较旧系统可分别设置。
