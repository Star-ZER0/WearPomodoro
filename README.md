...
- **正在进行的活动**：支持的 Wear OS 系统可在表盘显示番茄钟入口，并在启动器最近任务中显示当前阶段与倒计时，点按直接返回计时页。
- **实况动态（Live Updates）**：在支持的系统上请求突出显示计时通知，由系统显示实时倒计时；可在通知样式中选择。
...
| 屏幕样式 | 圆屏 | 圆屏 / 方屏 |
| 通知样式 | 正在进行的活动 | 实况动态 / 正在进行的活动 / 普通通知 |
...
**通用设置 → 通知样式**提供「实况动态」「正在进行的活动」「普通通知」三个单选项，默认选择正在进行的活动。实况动态与正在进行的活动不能同时使用，以避免系统通知中心产生重复入口而崩溃。选择会立即保存并更新当前通知，不改变本轮时长；不读取或迁移旧版开关值。前两种样式仅在学习或休息计时运行时显示，暂停或待开始时恢复为普通通知，继续后恢复所选样式。普通通知保留倒计时、操作按钮和阶段提醒。
...
正在进行的活动通过 [Wear OS Ongoing Activity API](https://developer.android.com/training/wearables/notifications/ongoing-activity) 提供，入口位置与倒计时展示取决于系统、启动器和表盘支持。未支持该 API 的设备仍可使用普通计时通知；显示活动需要允许应用通知并开启「番茄钟计时」通道。

## 权限与提醒

[Wear OS Live Updates](https://developer.android.com/training/wearables/notifications/live-updates) 从 Wear OS 7 起提供。实际展示由系统、设备厂商及系统通知设置决定；选择实况动态不代表系统一定会突出显示；系统不支持或未允许时显示普通通知，也可手动选择正在进行的活动。倒计时使用系统计时器，无需每秒重新发送通知。
...
macOS / Linux 将命令入口换为 `./gradlew`。Robolectric 测试覆盖正在进行的活动通知、倒计时、阶段切换和暂停恢复；系统表盘 / 启动器显示、点按返回计时页，以及涉及计时、通知、手势或触觉的修改，还应在目标设备上验证。功能和构建方式变化时，请同步更新两种语言的 README。
